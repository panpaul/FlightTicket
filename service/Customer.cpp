...
void Customer::PrintTickets()
{
	...
	for (auto i:orderVec)
	{
		...
		char mbstr[100];
		if (std::strftime(mbstr, sizeof(mbstr), "%a %H:%M:%S", std::localtime(&flightVec[0].Time)))
		{
			std::cout << "出发时间: " << mbstr << '\n';
		}
		std::cout << "-------------------------" << std::endl;
